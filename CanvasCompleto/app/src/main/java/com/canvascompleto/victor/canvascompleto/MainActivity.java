...
public class MainActivity extends ActionBarActivity {
    ...
    //AQUI PUEDEN IR LAS SUBLACES PARA BIBUJAR
    ...
    public class EjemploView extends View { // CLASE QUE EXTIENDE DE VIEW
        ...
        public EjemploView(Context contexto) {
            ...
            miDrawable.setBounds(30,430,330,580);  //CORDENADAS
        ...
        protected void onDraw(Canvas canvas) {
            ...
            for (int i=0;i<450;i+=150) { //1FILA JJOO
                ...
                canvas.drawCircle(350 + i, 800, 100, pinceluno);//COORDENADAS CENTRO (X,Y, RADIO)
            }
            for (int j=0;j<400;j+=200){ //2FILA JJOO
                if(j==0)pinceluno.setColor(Color.rgb(245,225,0));//COORDENADAS CENTRO (X,Y, RADIO)
                if(j==200)pinceluno.setColor(Color.rgb(35,196,13));
                canvas.drawCircle(400 + j, 880, 100, pinceluno);//COORDENADAS CENTRO (X,Y, RADIO)
            }
